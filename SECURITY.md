...
- **No automated test suite yet** for most of the above — [SECURITY_TESTING.md](SECURITY_TESTING.md) documents manual procedures; the DLP engine (`backend/tests/dlp.test.js` and `backend/tests/dlpConfidence.test.js`, run via `node --test`) is the first automated coverage, converting the remaining pure-function pieces (`riskEngine.js`, `policyEngine.js`) into an automated suite is a natural next step.
